...
export const generateNovelsBookmarks = () => {
  let novels = document.querySelectorAll('.novel');
  const userId = window.localStorage.getItem('userId');
  const bookmarkInterval = setInterval(async () => {
    novels = document.querySelectorAll('.novel');
    novels.length === 0 ? (novels = document.querySelectorAll('.bookmark-novel')) : '';
    if (novels.length !== 0) clearInterval(bookmarkInterval);
